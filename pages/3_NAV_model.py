    ...
    if st.button("Predict") and smiles_list:
        ...
        if results:
            ...
            if predictions:
                ...
                st.markdown(get_table_download_link(predictions_df, "Nav_predictions.csv"), unsafe_allow_html=True)

            if atom_importances:
                atom_imp_df = pd.concat(atom_importances)
                st.markdown(get_table_download_link(atom_imp_df, "Nav_atom_importance.csv"), unsafe_allow_html=True)

            if bond_importances:
                bond_imp_df = pd.concat(bond_importances)
                st.markdown(get_table_download_link(bond_imp_df, "Nav_bond_importance.csv"), unsafe_allow_html=True)
