...
class AttentiveFPModel(torch.nn.Module):
    def __init__(self):
        ...
        self.attentivefp = AttentiveFP(
            ...
            edge_dim=3, num_layers=6, num_timesteps=1, dropout=0.1
        )
...
def main():
    st.set_page_config(page_title="Kav_Predictor", layout="wide")
    st.title("Kav_Predictor - Potassium Channel Blocker Classification")
    st.markdown("""
    This app predicts whether a molecule is a hERG potassium channel blocker using an AttentiveFP model.
    """)
    ...
    if st.button("Predict") and smiles_list:
        ...
        if results:
            ...
            if predictions:
                ...
                st.markdown(get_table_download_link(predictions_df, "Kav_predictions.csv"), unsafe_allow_html=True)

            if atom_importances:
                atom_imp_df = pd.concat(atom_importances)
                st.markdown(get_table_download_link(atom_imp_df, "Kav_atom_importance.csv"), unsafe_allow_html=True)

            if bond_importances:
                bond_imp_df = pd.concat(bond_importances)
                st.markdown(get_table_download_link(bond_imp_df, "Kav_bond_importance.csv"), unsafe_allow_html=True)
